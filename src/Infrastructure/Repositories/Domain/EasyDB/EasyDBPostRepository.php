...
final class EasyDBPostRepository implements PostRepository
{
    ...
    public function updateTags(int $postId, array $tags): void
    {
        ...
        $allTags = collect($newTags)->merge($existedTags);
        $newRows = $allTags->map(fn (Tag $tag) => [
            'post_id' => $postId,
            'tag_id' => $tag->id,
        ])->values();

        if ($newRows->isNotEmpty()) {
            $this->db->delete('posts_tags', ['post_id' => $postId]);
            $this->db->insertMany('posts_tags', $newRows->toArray());
        }
    }
